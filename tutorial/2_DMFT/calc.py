...
dmft_param = {                 # DMFT loop parameters:
  ...
  "conv_iter": 3,              #  number of iterations where convergence criteria must all be satisfied (prevents premature exit from the DMFT loop)
  "max_iter": 50,              #  maximum number of iterations (signals convergence failure)
  "eps_prev": 1e-6,            #  convergence criterium: integrated diff between two consecutive local spectral functions
  "eps_loc_imp": 1e-6,         #  convergence criterium: integrated diff between local and impurity spectral function
  "eps_occupancy": 1e-5,       #  convergence criterium: difference from the target occupancy
  "mixing_method": "broyden",  #  "linear" or "broyden" mixing; the quantity being mixed is the hybridisation function (im part), and the chemical
                               #   potential mu of occup_method=broyden (see below)
  "alpha": 0.5,                #  mixing parameter from both linear and Broyden mixing, alpha=0.5 is a good default
  "occup_method": "broyden",   #  "adjust", "broyden" or None; adjust=shift mu so that the current GF meets the occupancy goal,
                               #   broyden=use mu/occupancy as an additional variable in the Broyden mixing
  "Delta_min": 1e-5,           #  minimal value for the hybridisation function; if too large, it produces incorrect spectral distribution,
                               #    f too small, it leads to discretization problems (1e-5 is usually a good and rather safe choice)
